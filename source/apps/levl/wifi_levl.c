...
static void get_probe_req_in_band(wifi_app_t *app, probe_req_elem_t *elem,
    frame_data_t *msg, ds_dlist_t *probe_req_frames_to_send)
{
    probe_frame_t *probe_req_frame = NULL;
...
static void extract_probe_req(wifi_app_t *app, probe_req_elem_t *elem,
    const mac_addr_str_t mac_str, ds_dlist_t *probe_req_frames_to_send)
{
    probe_frame_t *probe_req_frame = NULL;
    probe_frame_t *tmp_probe_req_frame = NULL;
    ...
    wifi_app_t *app, ds_dlist_t *probe_req_frames_to_send)
{
    ...
    for_each_element_extid(ie_elem, WLAN_EID_EXT_MULTI_LINK, (const u8 *)ie_ptr, ie_len)
    {
        ...
        if (common_info_len > ETH_ALEN) {
            ...
            if (elem != NULL) {
                wifi_util_dbg_print(WIFI_APPS,
                    "%s:%d: found matching ProbeReq with MLD MAC: %s\n", __func__, __LINE__,
                    mac_str);
                extract_probe_req(app, elem, mac_str, probe_req_frames_to_send);
            }
        }
...
#ifdef CONFIG_IEEE80211BE
            struct ieee80211_mgmt *probe = (struct ieee80211_mgmt *)probe_req_frame->probe_frame.data;
